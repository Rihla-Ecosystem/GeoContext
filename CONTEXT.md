...
- 2026-08-07: Sensitive-area feature: added `ZoneGuidance` schema + `nearby_zone_guidance` on ContextResponse; §6 ST_DWithin(geography) proximity query vs `restricted_zones` (limit 5, returns only zone_type+distance). Verified restart (port 8000, no --reload so must fuser -k after edits) → curl `/api/v1/context` with internal key returns guidance near Cairo, `[]` for far point. Note: pre-existing uvicorn held 8000 with stale code — kill by port before testing.- 2026-08-13: MAX_DETECTION_RADIUS bumped 5000 → 1,000,000 m (app/core/config.py) and DEPLOYED. Commit 910076f → CI Build & push image succeeded (run 31697003463) → docker pull ghcr.io/rihla-ecosystem/geocontext:latest (digest 02a948fe) → docker-compose --env-file ./env up -d --no-deps geocontext recreated rihla-geo. Verified: container Up (healthy), /healthz {"status":"ok"}, runtime Settings().MAX_DETECTION_RADIUS == 1000000.0. Local working copy still has the change (not reverted).
- 2026-08-19: Added `GET /api/v1/context/zones` — anonymous zone polygons (simplified GeoJSON, class + derived severity only) for map rendering; stripped identity fields (name/reason/source) from `AreaAdvisory`. Core only reads `advisory_type`, so shape change is safe. Pushed to org → CI image rebuild → deployed on VPS (rihla-geo).
